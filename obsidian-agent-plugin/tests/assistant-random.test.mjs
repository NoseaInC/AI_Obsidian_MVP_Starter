...
test("5000 seeded stream partitions reconstruct exact Markdown and normalize safely", async () => {
  ...
  for (let sample = 0; sample < 5_000; sample += 1) {
    ...
    state = stream.reduceAssistantStream(state, {schemaVersion: 1, seq: 1, type: "run.started", runId: `run-${sample}`, conversationId: "c"});
    chunks.forEach((delta, index) => {
      state = stream.reduceAssistantStream(state, {schemaVersion: 1, seq: index + 2, type: "message.delta", runId: `run-${sample}`, conversationId: "c", delta});
    });
