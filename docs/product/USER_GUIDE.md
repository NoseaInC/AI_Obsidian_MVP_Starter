...
- 助手顶部只保留当前会话标题。模型选择与联网状态位于输入框内；附件、当前笔记、Vault 笔记、命令、联网、上下文、历史、新会话和模型设置统一收进输入框左下角的 `+` 菜单。
- 点击输入框内的模型选择器可以切换“深度思考”。Auto 使用供应商默认推理强度；DeepSeek 的深度模式使用官方 `max` 推理强度，并提高输出预算、压缩过长旧上下文、强化证据与结果校验。推理内容只有在供应商真实返回 reasoning block 时才会出现，仍可折叠或展开。
...
- 助手由模型根据当前目标和真实 Observation 选择下一步工具，不使用关键词/正则 Intent Router。所有写入先生成 Change Set；Harness 对明确的低风险 Draft/Inbox 新建可自动应用，对已有笔记或更高风险操作在当前聊天中询问，对越权和 reviewed/core 修改直接阻止。
...
- 助手输入框显示紧凑的“本地/联网”状态。需要查公开资料时，从 `+` 菜单选择“开启联网”；授权只作用于当前会话并持久化该会话状态。
- 联网后，知序可以搜索通用网页、GitHub/Hacker News 资料以及 arXiv/Crossref 学术来源，也可以安全读取你粘贴的公开 HTTP(S) 链接。
- 助手会在同一个 Run 中完成“搜索 → 选择来源 → 抓取正文 → 重新规划 → 回答”。网页内容始终按不可信证据处理，不会执行网页里的指令。
- 右侧“来源”标签展示实际调用得到的标题、域名、Provider、质量和摘要。来源为空时会明确提示开启联网，不会把模型记忆伪装成联网结果。
- 联网搜索本身不会写入 Vault。若要求整理为笔记，仍会经过 Harness、Change Set、Diff 与权限策略；reviewed/core 继续只读。
- 直接粘贴 URL 会明确开启本会话联网并把该 URL 作为资料输入；API Key 不会进入请求、日志或来源卡片。
...
- 从输入框 `+` 菜单选择“新会话”会创建真正独立的会话；完成首个请求后会用该请求自动命名。“历史会话”可切换已有会话。
